...
class TechnicalIndicatorATAN(TechnicalIndicator):
    """
    ...
    def __init__(self, timeperiod=0):
        ...
        Returns:
            self
        """
        super(TechnicalIndicatorATAN, self).__init__()
        self.__timeperiod = timeperiod

    def _calc_indicator(self, OHLCV_input):
        """
        Calculates the ATAN - Vector Trigonometric ATan using a wrapper for the TA-lib
        ...
        output = DataFrame(ATAN(close))
        output.columns = ['ATAN%d' % self.__timeperiod]
        return output
