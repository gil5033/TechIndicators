...
class TechnicalIndicatorTRIMA(TechnicalIndicator):
    """
    Wrapper for the Triangular Moving Average period from TA-lib
    References:
    ...
    def __init__(self, timeperiod=30):
        ...
        Returns:
            self
        """
        super(TechnicalIndicatorTRIMA, self).__init__()
        self.__timeperiod = timeperiod

    def _calc_indicator(self, OHLCV_input):
        """
        Calculates the Triangular Moving Average technical indicator using a wrapper for the TA-lib
        ...
        output = DataFrame(TRIMA(close, self.__timeperiod))
        output.columns = ['TRIMA%d' % self.__timeperiod]
        return output
