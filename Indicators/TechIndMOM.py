...
class TechnicalIndicatorMOM(TechnicalIndicator):
    """
    Wrapper for the Momentum Indicator Rating from TA-lib
    References:
    ...
    def __init__(self, timeperiod=14):
        ...
        Returns:
            self
        """
        super(TechnicalIndicatorMOM, self).__init__()
        self.__timeperiod = timeperiod

    def _calc_indicator(self, OHLCV_input):
        """
        Calculates the Momentum Indicator technical indicator using a wrapper for the TA-lib
        ...
        output = DataFrame(MOM(close, self.__timeperiod))
        output.columns = ['MOM%d' % self.__timeperiod]
        return output
